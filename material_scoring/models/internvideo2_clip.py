...
class InternVideo2_CLIP(nn.Module):
    ...
    def self_sim(self, image, text_input, idx, prompt, is_text_prompt):
        image = image.flatten(0, 1)
    ...
    def cross_sim(self, image, text_input, idx, prompt, is_text_prompt):
        B, device = image.shape[0], image.device
        image = image.flatten(0, 1)

        self.clip_contrastive_temperature()
        vision_embeds = self.encode_vision(image)
        vision_embeds = F.normalize(vision_embeds, dim=-1).to(torch.bfloat16)
        ...
        if image_prompts:
            prompt_embeds[is_image_prompt] = self.encode_image(image_prompts).to(device)
        prompt_embeds = F.normalize(prompt_embeds, dim=-1).to(torch.bfloat16)
     
        print(self.prompt_loss.vtc_loss(
                vision_embeds, prompt_embeds, idx, self.temp, all_gather=True
            ))
        return (vision_embeds.transpose(0,1) @ prompt_embeds.transpose(-2,-1)).mean(dim=0)
