...
class ImgTxtPtTrainDataset(BaseDataset):
    ...
    def __init__(self, ann_file, transform, num_epochs=1):
        ...
        self.filter_caption_length = ann_file.get("filter_caption_length", False)
            ...
            if get_local_rank() == 0:  # Only one rank need to read the file
                ...
                if ann_file.get("jump_filter", False):
                    ...
                    if self.caption_augmentation is not None:
                        ...
                        annos = new_annos
                    elif self.filter_caption_length:
                        # filter out the caption with length less than min_caption_length
...
class VidTxtPtTrainDataset(ImgTxtPtTrainDataset):
    ...
    def get_anno(self, index):
        ...
        anno = {"prompt": self.anno[index]["prompt"]}
        if "caption" in self.anno[index].keys():
            anno["caption"] = self.get_caption(index)
            if self.use_prompt:
                anno["caption"] = random.choice(self.prompt).format(anno["caption"])
        if "score" in self.anno[index].keys():
            anno["score"] = self.anno[index]["score"]
        if isinstance(self.anno[index]["video"], list):
            anno["video"] = [self.data_root_prefix + os.path.join(self.data_root, v) for v in self.anno[index]["video"]]
        else:
    ...
    def __getitem__(self, index):
        try:
            ann = self.get_anno(index)
            caption = self.pre_caption(ann["caption"]) if "caption" in ann.keys() else ""
            score = torch.from_numpy(np.array(ann["score"],dtype=np.float32)) if "score" in ann.keys() else []
            ...
            if os.path.isfile(ann["prompt"]):
                is_text_prompt = False
                prompt = ann["prompt"]
            else:
                is_text_prompt = True
                prompt = self.pre_caption(ann["prompt"])

            video, index = self.load_and_transform_media_data(index, data_path)

            return video, caption, index, prompt, is_text_prompt, score
